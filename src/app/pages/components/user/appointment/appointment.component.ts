...
export class AppointmentComponent implements AfterViewInit  {
    ...
    displayedColumns: string[] = ['product', 'customer', 'price', 'date', 'status']; //'vendor',
    displayedColumnsExpan: string[] = ['Peluqueria', 'Estado'];
    columnsToDisplayWithExpand: string[] = [...this.displayedColumnsExpan, 'delete', 'add', 'expand'];
    ...
    closeAppoinment(el:any){
        ...
        this._getAppointment.deleteCompleteAppointment(body)
            .subscribe(
                (response) => {
...
export class UpdateAppointmentDialogBox implements  OnInit {
    ...
    SelectCollaborator(){
        this.BusinessSelected = this.Business.filter((bs:any) => bs._id == Business.business.business)
        console.log(this.BusinessSelected )
        this.colaboradores = this.BusinessSelected[0].branchoffices[0].collaborators.filter((col:any) => col.profile[0].code !== "MANAGER_ROLE" && col.services.some((el:any) => el.services == this.updateForm.get('services')?.value))
    }
    ...
    valueChanged(e:any){
        this.getHours();
        this.ServicesColaborator = this.updateForm.get('staff')?.value.typeService;
    }
